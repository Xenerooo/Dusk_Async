...
func load_scene_async_with_cb(target : Node, path : String, pos : Vector2, is_pos_global : bool, cb : FuncRef, data : Dictionary, has_priority := false) -> void:
	var entry := {
		"path" : path,
		"cb" : cb,
		"pos" : pos,
...
func load_scene_async(target : Node, path : String, pos : Vector2, is_pos_global : bool) -> void:
...
#		cb.call_deferred("call_func", path, instance, pos, is_pos_global, data)
		...
		if pos != Vector2.INF in instance:
			# Convert the position from global to local if needed
			if is_pos_global:
				pos = pos - target.global_position
