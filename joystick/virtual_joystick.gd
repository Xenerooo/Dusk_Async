...
export (float, 0, 1, .1) var active_zone :float = 0.1
...
enum functionality {AIMING, MOVEMENT}


export(functionality) var funcionality_mode : = functionality.MOVEMENT
...
func _update_joystick(touch_position: Vector2) -> void:
	...
	if vector.length_squared() > deadzone_size * deadzone_size:
		...
		if _output.length() > active_zone :
			_active = true
		elif  _output.length() < active_zone : 
			_active = false
