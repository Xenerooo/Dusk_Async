...
"res://Assets/scenes/locations/World.gd"var previous_chunk = Vector2()
...
#	current_chunk = _get_player_chunk(player.global_position)
	current_chunk = get_global_mouse_position()
	...
	for x in range(render_bounds):
		for y in range(render_bounds):
			...
			if active_coord.find(chunk_coords) == -1:
				var chunk = preload("res://Assets/scenes/locations/chunk.tscn").instance()
